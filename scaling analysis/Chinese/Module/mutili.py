...
If you want to see every picture of your txt, you should use Run_case_by_case.ipynb and DO NOT import this module.
...
def which_plot(name, V, H, x = 'H', SP = 'T', FORMAT = 'png', max_range = 50, shift = 'N', Path = ''):
    ...
    2. V, H: list or np.array
       ...
       where geometric_sequence(word, char) is the function of count.py
    ...
    if x == 'H':
        ...
        plt.plot(r_position, r, 'ro')        
        if (SP == 'T') and (FORMAT == 'png' or 'pdf' or 'ps' or 'eps' or 'svg'):
            fig.savefig(Path + 'V of ' + name + '.' + FORMAT, dpi = 1000, format = FORMAT)
        plt.close()
    else:
        print('please chose x = \'H\' or \'V\'')

def RRD_plot(big, word, char, longest, name, V, H, need_line = 'Y', number_of_lines = 4, Color = '#ff0000', SP = 'T', FORMAT = 'png', Path = ''):
    ...
    if (SP == 'T') and (FORMAT == 'png' or 'pdf' or 'ps' or 'eps' or 'svg'): 
        ...
        for i in range(longest): #draw 0th_char ~ longest_th_char on RRD plot
            ...
            plt.plot(str_position, big[str(i) + "th_char_rank"], 'o', markersize=3, color = Color)
        ...
        plt.title(name, size = 20)    
        fig.savefig(Path + 'RRD of ' + name + '.' + FORMAT, dpi=1000, format = FORMAT) #adjust dpi if you want the figure more clear
        plt.close()
    else:
        print('no RRD plot.')
    
def FRD_plot(name, word, char, x_pos = 2, y_pos = 1, SP = 'T', FORMAT = 'png', Path = ''):
    '''draw FRD plot of words and characters

       parameters:
    0. name: str
       "XXX" (your file name without filename extension)

    1. word, char: pd.daframe
    ...
    if (SP == 'T') and (FORMAT == 'png' or 'pdf' or 'ps' or 'eps' or 'svg'):
        ...
        plt.plot(cf, 'x', label = 'char', markersize=6)
        plt.legend(loc = 'best', prop={'size': 20})
        ax.tick_params(axis='x', labelsize=15) 
        ax.tick_params(axis='y', labelsize=15)
        #https://stackoverflow.com/questions/6774086/why-is-my-xlabel-cut-off-in-my-matplotlib-plot
        plt.gcf().subplots_adjust(left = 0.17, bottom = 0.17)
        fig.savefig(Path + 'FRD of ' + name + '.' + FORMAT, dpi = 300, format = FORMAT)
        plt.close()
    else:
        print('no FRD plot.')

def Col_plot(name, char, x_pos = 10, y_pos = 0, SP = 'T', FORMAT = 'png', Path = ''):
    ...
    '''
    if (SP == 'T') and (FORMAT == 'png' or 'pdf' or 'ps' or 'eps' or 'svg'):
        Char = char.sort_values(by = '#collocations', ascending=False)
        reChar = Char.reset_index()
        ...
        popt, pcov = curve_fit(col, char['charRank'], reChar['#collocations'])
        #popt is the optimal values for the parameters (a,b)
        theo = col(char['charRank'], *popt)
        fig, ax = plt.subplots()
        plt.plot(char['charRank'], theo, 'g--')

        a = 7   #auto positioning, m = min(char['charRank']) = 1 always
        b = 1   #auto positioning, M = max(char['charRank'])
        xmid = max(char['charRank'])**(b/(a+b))  #exp([a*log(m)+b*log(M)]/[a+b]) = m^(a/[a+b]) * M^(b/[a+b])    
        ytop = max(reChar['#collocations'])*5/6
        ...
        #-----------------------------------------
        col_dig = len(str(max(reChar['#collocations'])))
        yp_dig = len(str(max(char['charRank']))) #ln(y') will have yp_dig +1 digits (yp_dig significant figures)
        ...
        plt.plot(reChar['#collocations'], 'ro', label = 'char', markersize = 4)
        plt.xlabel('rank of chars($y\prime$)', size = 20)
        plt.xscale('log')
...
def Link_plot(name, word, x_pos = 20, y_pos = 0, SP = 'T', FORMAT = 'png', Path = ''):
    ...
    if (SP == 'T') and (FORMAT == 'png' or 'pdf' or 'ps' or 'eps' or 'svg'):
        ...
        a = 7   #auto positioning, m = min(word['wordRank'])
...
def Scaling_fit(data, Rf, V, H, name, SP, FORMAT, Path = ''):
    ...
    for N in range(1, number + 1):
        ...
        plt.plot(data[FN][0], data[FN][1], 'o', markersize = '3')
        plt.plot(data[FN][0], theo[FN], 'o', markersize = '3', color ='yellow')
