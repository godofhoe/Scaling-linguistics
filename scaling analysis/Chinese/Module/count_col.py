...
def count_col(pdframe1, pdframe2, feature1 = "word", feature2 = "char", feature3 = "charFreq"):
    '''count the collocations of characters and the links of words
    ...
    char_array = pdframe2[feature2] #ex: char_array=['你', '囉', '好', '哈']
    ...
    #add a frame "#collocations" (numbers of collocations of chars) to char
...
def chose_point(m, n, V, H, big, longest):
    ...
    for j in range(min(len(V) - n, len(H) - m)):
        ...
        for i in range(longest): #draw 0th_char ~ (longest-1)_th_char
            H_char = V_word[(V_word[str(i) + "th_char_rank"] <= H[j+m-1]) & (V_word[str(i) + "th_char_rank"] > H[j+m])]
            ind = H_char.index #if I don't use index here, the content of points will be series not value
            for k in ind:
                points[j].append((H_char.loc[k, 'wordRank'], H_char.loc[k, str(i) + "th_char_rank"]))
    return points
...
def scaling_fit(data, Rf, V, H, name, SP, FORMAT):
    ...
    for N in range(1, number + 1):
        ...
        plt.plot(data[FN][0], data[FN][1], 'o', markersize = '3', color ='yellow')
        plt.plot(data[FN][0], theo[FN], 'o', markersize = '3')
    xm, xM = plt.xlim([0,V[0]*1.03])
    ym, yM = plt.ylim([0,H[0]*1.03])
    plt.text(0.05*xM, 0.05*yM, '$f_%d=%.2f*%.2f^{%.2f x^{-%.2f}}$' % (n, popt[0], Rf, popt[1], popt[2]), fontsize=24, color ='black')
    plt.xlabel('word', size = 15)
    plt.ylabel('character', size = 15)  
    plt.title(name, size = 20)
